...
from apollo_sim.bridge.apollo.cyber_bridge import CyberBridge
from apollo_sim.bridge.apollo.dreamview import Dreamview
from apollo_sim.tools.global_data import GlobalData
...
class ApolloContainer:
    ...
    def create_container(self):
        ...
        if not has_create:
            ...
            start_script_path = os.path.join(os.path.join(os.path.dirname(__file__), 'scripts', 'dev_start_ctn.sh'))
            options = "-y -l -f"
    ...
    ##### Dreamview Operation #####
    def start_dreamview(self, hd_map, dv_mode= 'Mkz Standard Debug', apollo_type='Mkz_Example', wait_time=3.0, restart=False) -> bool:
        # setup dreamview
        if restart:
            cmd_op = 'restart'
        else:
...
#     print(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'scripts', 'dev_start_ctn.sh'))